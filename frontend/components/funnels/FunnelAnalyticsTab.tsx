    ...
    <div className="space-y-6">
      ...
      {analytics.step_counts.length === 0 && (
        <div className="bg-white rounded-lg shadow p-12 text-center">
          <p className="text-gray-600">No analytics data available. Start tracking events to see analytics.</p>
        </div>
      )}
      ...
      {analytics.top_utm_sources && analytics.top_utm_sources.length > 0 && (
        <div className="bg-white rounded-lg shadow overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-200">
            <h3 className="text-lg font-semibold text-gray-900">Top UTM Sources</h3>
            <p className="text-sm text-gray-600 mt-1">Traffic sources by UTM parameter</p>
          </div>
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {analytics.top_referrers.map((ref, index) => (
                <tr key={index}>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                    {ref.referrer === 'Direct' ? (
                      <span className="text-gray-500 italic">Direct (no referrer)</span>
                    ) : (
                      <a
